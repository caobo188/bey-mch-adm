  ...
  <div class="list">
    <header>
      ...
      <el-select v-model="heroPosition" class="formInputCss">
        <el-option v-for="item in heroPositions" :key="item.value" :label="item.label" :value="item.value">
        </el-option>
      </el-select>

      <label for="" class="formLabelCss">性别:</label>
      <el-select v-model="heroSex" class="formInputCss">
        <el-option v-for="item in heroSexs" :key="item.value" :label="item.label" :value="item.value">
        </el-option>
      </el-select>
    ...
    <main>
      <el-table :data="tableData" stripe element-loading-text="拼命加载中" header-row-class-name="tableHeader" v-loading.fullscreen.lock="loading" empty-text="亲，暂时没有数据哦" border style="width: 100%">
        <el-table-column fixed prop="heroName" label="英雄" align="center" width="120">
        </el-table-column>
        ...
        <el-table-column label="性别" align="center" width="100">
          <template slot-scope="scope">
            ...
            <i class="iconfont myIcon-woman" v-if="scope.row.heroSex=='woman'"></i> {{jungleSex(scope.row.heroSex)}}
          </template>
        </el-table-column>
        <el-table-column prop="address" label="籍贯" align="center" width="150">
        </el-table-column>

        <el-table-column prop="dowhat" label="位置" align="center" width="163">
      ...
      <el-pagination v-if="paginationShow" class="pagination" @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-size="3" layout="total, prev, pager, next, jumper" :total="length">
      </el-pagination>
    </main>
